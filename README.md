...
By leveraging an offline, zero-cost stack, this environment demonstrates deterministic context isolation, strict boundary enforcement, and precise citation tracking before scaling to enterprise AWS infrastructures (**Amazon Bedrock**, **Amazon OpenSearch Serverless**, and **AWS Lambda**).
...
### 2. The Inference Pipeline (Real-Time Deterministic Loop)

```text
[User/Clinician Query] 
       │
       ▼
[Generate Query Embedding] ──► [ChromaDB Spatial Search] ──► [Extract Top-K Context + Metadata]
                                                                          │
                                                                          ▼
[Clinician Screen] ◄── [Output Generation] ◄── [Llama 3 Inference] ◄── [Strict System Prompt Guard]
```
...
| **Security Aduna** | Explicit System Prompt Restrictions | **Amazon Bedrock Guardrails** | Automated Contextual Grounding Checks |
